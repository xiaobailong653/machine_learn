...
class ConvLayer(object):
    ...
    def bp_sensitivity_map(self, sensitivity_array, activator):
        ...
        # full卷积，对sensitivity map 进行zero padding
        # 虽然原始输入的zero padding单元也会获得残差
        # 单这个残差不需要继续向上传递，因此就不计算了
        ...
        # 对于具有多个filter的卷积层来说，最终传递到上一层的sensitivity map 相当于所有的filter的sensitivity map之和
        for f in range(self.filter_number):
            filter_item = self.filters[f]
            flipped_weights = np.array(map(lambda i: np.rot90(i, 2), filter_item.get_weights()))
    ...
    def forward(self, input_array):
        ...
        for f in range(self.filter_number):
            filter_item = self.filters[f]
            conv(self.padded_input_array, filter_item.get_weights(), self.output_array[f], self.stride, filter_item.get_bias())
    ...
    def bp_gradient(self, sensitivity_array):
        ...
        for f in range(self.filter_number):
            # 计算每个权重的梯度
            filter_item = self.filters[f]
            for d in range(filter_item.weights.shape[0]):
                conv(self.padded_input_array[d], expanded_array[f], filter_item.weights_grad[d], 1, 0)

            # 计算偏置项的梯度
            filter_item.bias_grad = expanded_array[f].sum()

    def update(self):
        '''按照梯度下降，更新权重'''
        for filter_item in self.filters:
            filter_item.update(self.learning_rate)
    ...
    if zp == 0:
        ...
        if input_array.ndim == 3:
            ...
            padded_array = np.zeros((
                ...
                input_height + 2 * zp,
                input_width + 2 * zp))
            padded_array[:,zp:zp + input_width, zp:zp + input_width] = input_array
            return padded_array
...
def gradient_check():
    ...
    # 检查梯度
    epsilon = 10e-4
    for d in range(cl.filters[0].weights_grad.shape[1]):
